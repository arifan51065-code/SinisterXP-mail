...
# SinisterXP Mail Bot — Full Stable Version (All Admin Cmds + Reset Fix + Git Backup + Keepalive)
...
def _backup_once():
    if not (GITHUB_TOKEN and GITHUB_REPO): return True
    _git_bootstrap()
    ...
    if ok_push: log.info("Backup pushed: %s", os.path.basename(dst))
    return True
